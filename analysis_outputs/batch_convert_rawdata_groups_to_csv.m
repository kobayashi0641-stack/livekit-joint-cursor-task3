% Batch convert RawData/g## JSON recordings to AnalyzedData/g##_data.csv.
%
...
%
% Output:
%   <projectFolder>/AnalyzedData/g1_data.csv
%   <projectFolder>/AnalyzedData/g2_data.csv
%   ...
%
% CSV format:
...
%   "mergeSameExperiment" joins split JSON files with the same experimentName.
%   "latest" converts only the most recently modified JSON.
%   "error" stops and asks you to resolve ambiguity.
multipleJsonPolicy = "mergeSameExperiment";
...
for groupNo = groupStart:groupEnd
    ...
    if numel(jsonFiles) > 1
        ...
        elseif multipleJsonPolicy == "latest"
            ...
            jsonFiles = jsonFiles(idx);
            warning("Multiple JSON files found in %s. Using latest: %s", groupFolder, jsonFiles.name);
        elseif multipleJsonPolicy == "mergeSameExperiment"
            fprintf("[%s] Found %d JSON files. Will merge files with the same experimentName.\n", ...
                groupName, numel(jsonFiles));
        else
    ...
    outCsvPath = fullfile(analyzedDataFolder, groupName + "_data.csv");

    fprintf("\n[%s] Converting:\n%s\n", char(groupName), char(strjoin(sourceJsonPaths, newline)));

    try
        [T, identityMap, samplingSummary, mergedInfo] = convertCursorControlJsonToTable(sourceJsonPaths);
        writetable(T, outCsvPath);

        outMapPath = fullfile(analyzedDataFolder, groupName + "_identity_map.csv");
        writetable(identityMap, outMapPath);

        newRow = table( ...
            groupName, strjoin(sourceJsonPaths, " | "), string(outCsvPath), height(T), ...
            ...
            'VariableNames', {'groupName', 'sourceJsonPath', 'outCsvPath', 'nRows', ...
            'meanIntervalMs', 'medianIntervalMs', 'estimatedHzFromMean', 'estimatedHzFromMedian'});
        batchSummary = [batchSummary; newRow]; %#ok<AGROW>

        fprintf("[%s] Wrote %d rows to:\n%s\n", char(groupName), height(T), outCsvPath);
        fprintf("[%s] Converted trials: %s\n", char(groupName), mat2str(mergedInfo.trialNumbers(:)'));
        fprintf("[%s] Identity map:\n%s -> test1\n%s -> test2\n", ...
            char(groupName), identityMap.OriginalIdentity(1), identityMap.OriginalIdentity(2));
    catch ME
        warning("[%s] Failed: %s", char(groupName), ME.message);
    end
end
...
function [T, identityMap, samplingSummary, mergedInfo] = convertCursorControlJsonToTable(jsonPaths)
    ...
    T = table( ...
        ...
        frameNumber, timestamp, timeFromTrialStartSec, frameRateDeclared, cursorDisturbanceType, ...

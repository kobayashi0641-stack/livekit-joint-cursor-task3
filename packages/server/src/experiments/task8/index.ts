...
function buildDisturbance(
  ...
  return {
    ...
    type: 'axis-aligned-gain-plus-rotation',
    phase,
...
export const task8Task: ExperimentTask = {
  type: 'task8',
  ...
  generateTrialSequence(config: ExperimentConfig, experimentName: string): AgentRule[] {
    ...
    const adaptation = Math.max(0, Math.floor(config.cursorControlAdaptationTrials));
    ...
    const total = baseline + adaptation + shared + washout;
    ...
    for (let i = 0; i < adaptation; i++) {
      rules.push({
        ...buildExecuteTrialRule(config, experimentName, trialNumber++, total),
        displayMode: 'avgOnly',
        sharedPhase: 'adaptation',
        sharedDisturbance: buildDisturbance(config, 'adaptation', i),
        recordingUploadTimeoutSeconds: 1,
      });
    }
    ...
    if (shared > 0) {
      rules.push({
        ...
        text: 'From the next trial, you will control a shared cursor with your partner. You can only move it forward and backward, while your partner will control it left and right.',
        instructionPages: [
          'From the next trial, you will control a shared cursor with your partner. You can only move it forward and backward, while your partner will control it left and right.',
        ],
        durationMs: config.instructionDurationMs,
        ...
        unlockPointerLock: true,
      });
      rules.push({ type: 'enableVirtualCursor', timeoutSeconds: 0 });
      rules.push({ type: 'setClickAreaOverlay', show: false });
    }
    ...
    for (let i = 0; i < shared; i++) {
      rules.push({
        ...
        displayMode: 'avgOnly',
        sharedPhase: 'shared',
        sharedDisturbance: buildDisturbance(config, 'shared', adaptation),
        recordingUploadTimeoutSeconds: 1,
      });
    }
    ...
    for (let i = 0; i < washout; i++) {
      rules.push({
        ...
        displayMode: 'self',
        sharedPhase: 'washout',
        sharedDisturbance: buildDisturbance(config, 'washout', adaptation),
        recordingUploadTimeoutSeconds: 1,
      });
    }
